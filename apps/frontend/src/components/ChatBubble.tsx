...
const ChatBubble = ({message, user}: Props) => {
  ...
  return (
    <div
      className={`max-w-[70%] px-3.5 py-2.5 rounded-2xl text-[15px] leading-snug ${message.sender.id === user.id ? 'bg-accent text-white rounded-br' : 'bg-code-bg text-text-h rounded-bl'}`}
    >
